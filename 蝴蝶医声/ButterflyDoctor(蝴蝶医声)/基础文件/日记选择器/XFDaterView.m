...
- (void)initData{
    ...
    if ([GATO_XFData isEqualToString:@"1"]) {
        self.yearNumber = 0;
        self.startYearNumber = -100;
        self.leftButtonName = @"保 密";
    }else{
        self.yearNumber = 10;
        self.startYearNumber = 0;
        self.leftButtonName = @"取 消";
    }
    [cancelBtn setTitle:self.leftButtonName forState:UIControlStateNormal];
    for (NSInteger i = self.startYearNumber; i<self.yearNumber; i++) {
        NSString *str = [NSString stringWithFormat:@"%ld",i + _year];
...
- (void)initViews{
    ...
    daterContentView = [[UIView alloc]initWithFrame:Frame(WH(20), WH(200), SCREEN_WIDTH-WH(40), WH(200))];
    ...
    daterLab.font=Font(20);
    daterLab.textColor = WhiteColor;
    daterLab.textAlignment=1;
    [daterContentView addSubview:daterLab];
    
    picker = [[UIPickerView alloc]initWithFrame:Frame(WH(20), daterLab.h, SCREEN_WIDTH-WH(80), WH(130))];
    ...
    UITapGestureRecognizer *tapRecognize = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(fadeOut)];
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        if (self.YearBool == NO) {
            if ([GATO_XFData isEqualToString:@"1"]) {
                self.dateString = [NSString stringWithFormat:@"1967-%02d-%02d",_month,_day];
            }else{
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    [rowLab setFont:[UIFont systemFontOfSize:20]];
    if (_dateViewType == XFDateViewTypeDate) {
        if(component == 0){
