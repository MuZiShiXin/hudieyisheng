...
- (void)configerToolBars{
    UILabel * pageNumlable = [[UILabel alloc] initWithFrame:CGRectMake(kUIScreenWidth/2 - 40, kUIScreenHeight - 50, 80, 30)];
...
//    pageNumlable.backgroundColor = RGBColor(0, 0, 0); 
    ...
    removeBtn.frame = CGRectMake(0, kUIScreenHeight - 40, Gato_Width / 2, 40);
    [removeBtn setTitle:@"返回" forState:UIControlStateNormal];
    removeBtn.titleLabel.font = [UIFont systemFontOfSize:16];
//    removeBtn.layer.cornerRadius = 5;
    [removeBtn setBackgroundColor:[UIColor HDThemeColor]];
    [removeBtn setTitleColor:RGBColor(255, 255, 255) forState:UIControlStateNormal];
    [removeBtn addTarget:self action:@selector(removeImage) forControlEvents:UIControlEventTouchUpInside];
//    [self insertSubview:removeBtn aboveSubview:self.scrollView];
    
    UIView * fgx = [[UIView alloc]initWithFrame:CGRectMake(Gato_Width / 2 - 1, 10, 1, 20)];
    fgx.backgroundColor = [UIColor appAllBackColor];
    [removeBtn addSubview:fgx];
    
    UIButton * saveBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    saveBtn.frame = CGRectMake(Gato_Width / 2, kUIScreenHeight - 40, Gato_Width / 2, 40);
    [saveBtn setTitle:@"保存" forState:UIControlStateNormal];
    saveBtn.titleLabel.font = [UIFont systemFontOfSize:16];
//    saveBtn.layer.cornerRadius = 5;
    [saveBtn setBackgroundColor:[UIColor HDThemeColor]];
...
- (void)setUpSubViews{
    ...
    for (int i = 0; i < _imagesUrlAry.count; i++) {
        UIScrollView * smallScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(kUIScreenWidth * i, 0, kUIScreenWidth, kUIScreenHeight - 40)];
