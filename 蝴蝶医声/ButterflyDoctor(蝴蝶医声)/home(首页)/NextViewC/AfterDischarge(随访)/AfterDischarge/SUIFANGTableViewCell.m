...
@interface SUIFANGTableViewCell ()
...
{
    static NSString *ID = @"SUIFANGTableViewCell";
    SUIFANGTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        // 从xib中加载cell
        cell = [[[NSBundle mainBundle] loadNibNamed:@"SUIFANGTableViewCell" owner:nil options:nil] lastObject];
...
-(void)setValueWithModel:(AfterDischargeModel *)model
{
    ...
    self.height = Gato_Height_548_(168) + labelHeight;
}
- (void)awakeFromNib {
    [super awakeFromNib];
...
//    self.backgroundColor = [UIColor appAllBackColor];
    ...
    .rightSpaceToView(self.underView,0)
    .topSpaceToView(self.yihuangoutong,Gato_Height_548_(0))
    .heightIs(Gato_Height_548_(1));
    
    self.closeButton.sd_layout.topSpaceToView(self.yihuangoutong, Gato_Height_548_(10))
