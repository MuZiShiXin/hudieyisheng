...
@interface makeArticleOneTableViewCell ()<UITextViewDelegate>
...
@end
@implementation makeArticleOneTableViewCell


+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    static NSString *ID = @"makeArticleOneTableViewCell";
    makeArticleOneTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        // 从xib中加载cell
        cell = [[[NSBundle mainBundle] loadNibNamed:@"makeArticleOneTableViewCell" owner:nil options:nil] lastObject];
...
- (void)awakeFromNib {
    ...
    self.textViewPlaceholder.font = FONT(36);
    self.textViewPlaceholder.textColor = [UIColor YMAppAllTitleColor];
    self.textViewPlaceholder.text = @"请输入文章小标题";
