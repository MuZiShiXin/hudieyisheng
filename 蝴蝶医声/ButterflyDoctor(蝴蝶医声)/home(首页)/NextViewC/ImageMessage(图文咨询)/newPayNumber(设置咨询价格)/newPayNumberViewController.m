...
@interface newPayNumberViewController ()<UIPickerViewDataSource,UIPickerViewDelegate,UIActionSheetDelegate>
//遵循协议
...
@implementation newPayNumberViewController
...
-(void)updateLoad
{
    ...
    [IWHttpTool postWithURL:HD_Home_Old_Payprice params:self.updateParms success:^(id json) {
        
        NSDictionary * dic = [NSJSONSerialization JSONObjectWithData:json options:NSJSONReadingAllowFragments error:nil];
        NSString * success = [dic objectForKey:@"code"];
        if ([success isEqualToString:@"200"]) {
            NSString * payStr = [[[dic objectForKey:@"data"] objectForKey:@"info"] objectForKey:@"payPrice"];
            self.payPriceStr = payStr > 0 ? payStr : @"9";
            self.moneryLabel.text = [NSString stringWithFormat:@"%@ 元 ／ 3次",self.payPriceStr];
            
            self.notDisturbStr = [[[dic objectForKey:@"data"] objectForKey:@"info"] objectForKey:@"notDisturb"];
            ...
            for (int i = 1 ; i < 8; i ++) {
                ...
                if ([self.notDisturbStr containsString:[NSString stringWithFormat:@"%d",i ]] || [self.notDisturbStr containsString:titleArray[i - 1]]) {
                    model.select = YES;
                }else{
                    model.select = NO;
                }
            ...
            if (self.notDisturbStr.length > 0) {
                self.notDisturbStr = [self.notDisturbStr stringByReplacingOccurrencesOfString:@"1"withString:@"每周一"];
                self.notDisturbStr = [self.notDisturbStr stringByReplacingOccurrencesOfString:@"2"withString:@"每周二"];
                self.notDisturbStr = [self.notDisturbStr stringByReplacingOccurrencesOfString:@"3"withString:@"每周三"];
                self.notDisturbStr = [self.notDisturbStr stringByReplacingOccurrencesOfString:@"4"withString:@"每周四"];
                self.notDisturbStr = [self.notDisturbStr stringByReplacingOccurrencesOfString:@"5"withString:@"每周五"];
                self.notDisturbStr = [self.notDisturbStr stringByReplacingOccurrencesOfString:@"6"withString:@"每周六"];
                self.notDisturbStr = [self.notDisturbStr stringByReplacingOccurrencesOfString:@"7"withString:@"每周日"];
                self.restDay.text =  [self.notDisturbStr substringToIndex:[self.notDisturbStr length] - 1];
            }else{
                self.restDay.text = @"";
            }
...
-(void)searchprogram
{
    self.updateParms = [NSMutableDictionary dictionary];
    [self.updateParms setObject:TOKEN forKey:@"token"];
    [self.updateParms setObject:self.payPriceStr forKey:@"payPrice"];
    [self.updateParms setObject:ModelNull(self.notDisturbStr) forKey:@"notDisturb"];
    [IWHttpTool postWithURL:HD_Home_Payprice params:self.updateParms success:^(id json) {
        ...
        if ([success isEqualToString:@"200"]) {
            ...
            if (self.payPriceBlock) {
                self.payPriceBlock(self.payPriceStr,ModelNull(self.notDisturbStr));
            }
...
//                                                         } animated:YES];
        ...
        //日期
        //Gato_Width_320_(24),Gato_Height - ( Gato_Height_548_(35 ) * 7 + Gato_Height_548_(50) ), Gato_Width_320_(274),Gato_Height_548_(35 ) * 7 + Gato_Height_548_(50)
        [PellTableViewSelect addPellTableViewSelectWithwithView:self.restView
                                                    WindowFrame:CGRectMake(Gato_Width_320_(24), Gato_Height - ( Gato_Height_548_(35 ) * 7 + Gato_Height_548_(50) ),Gato_Width_320_(274), Gato_Height_548_(35 ) * 7 + Gato_Height_548_(50))
                                                  WithViewFrame:CGRectMake(0, 0, Gato_Width,Gato_Height)
                                                     selectData:nil
                                                         action:^(NSInteger index) {
                                                             ;
                                                         } animated:YES];
...
{
    if (!_restView) {
        _restView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, Gato_Width, Gato_Height_548_(35 ) * 7 + Gato_Height_548_(50))];
        ...
        .heightIs(Gato_Height_548_(35 ) * 7);
