...
+ (instancetype)czh_itemWithTitle:(NSString *)title style:(CZHAlertItemStyle)style handler:(void (^)(CZHAlertItem *))handler {
    ...
    return [self czh_itemWithTitle:title titleFont:CZHGlobelNormalFont(13) titleColor:[UIColor redColor] backgroundColor:CZHColor(0xffffff) style:style handler:handler];
}
...
+ (void)initialize {
    ...
    appearance.messageColor = Gato_(0, 0, 0);
    appearance.titleFont = CZHGlobelNormalFont(17);
    appearance.messageFont = CZHGlobelNormalFont(14);
...
- (void)czh_calculateFrameForAlert {
    ...
    CGFloat wholeContainerViewW = CZH_ScaleWidth(280);
    ...
    if (self.itemArray.count > 0) {
        ...
        CGFloat buttonHeight = CZH_ScaleWidth(40);
        ...
        for (NSInteger i = 0; i < itemCount; i++) {
            ...
            } else if (itemCount==3)//三个按钮一排
            {
                buttonW = buttonContainerViewW * 1.0 / 3;
                buttonX = buttonW * i;
                buttonY = buttonContainerViewH;
            }else {
    ...
    [_buttonContainerView cornerAllCornersWithCornerRadius:CZH_ScaleWidth(5)];
    
    wholeContainerViewY = (self.czh_height - wholeContainerViewH) * 0.4;
...
//sheet类类型
        CGFloat buttonHeight = CZH_ScaleWidth(45);
        CGFloat lineHeight = 0.5;
        ...
        if (cancelItemCount > 0) {
            ...
            CGFloat buttonX = 0;
            CGFloat buttonY = buttonContainerViewH + CZH_ScaleWidth(8);
            CGFloat buttonW = buttonContainerViewW;
            CGFloat buttonH = buttonHeight;
            CZHWeakSelf(self);
            CZHAlertButton *button = [CZHAlertButton czh_alertbuttonWithTitle:item.title titleColor:item.titleColor titleFont:item.titleFont backgroundColor:item.backgroundColor buttonHandler:^(CZHAlertButton *button) {
