...
@interface EaseMessageCell()
{
    ...
    NSInteger * LongTagInteger;//及时用 只有当0时进行方法 其他return；
}
...
//                _bubbleView.imageView.image = [UIImage imageNamed:@"EaseUIResource.bundle/imageDownloadFail"];
    ...
    UILongPressGestureRecognizer * longPressGesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(LongAvatarViewTapAction:)];
//    longPressGesture.minimumPressDuration = 1.0;//不好使
    LongTagInteger = 0;
    [_avatarView addGestureRecognizer:longPressGesture];
}
        ...
        switch (model.bodyType) {
            ...
            case EMMessageBodyTypeImage:
            {
                ...
                if (!image) {
                    image = _model.image;
                    if (!image) {
                        [_bubbleView.imageView sd_setImageWithURL:[NSURL URLWithString:_model.fileURLPath] placeholderImage:[UIImage imageNamed:@""]];
                    } else {
            ...
            case EMMessageBodyTypeVideo:
            {
                UIImage *image = _model.isSender ? _model.image : _model.image;
                if (!image) {
                    image = _model.image;
                    if (!image) {
...
- (void)LongAvatarViewTapAction:(UILongPressGestureRecognizer *)tapRecognizer
{

    if (LongTagInteger == 0) {
        if ([_delegate respondsToSelector:@selector(LongAvatarViewSelcted:)]) {
            [_delegate LongAvatarViewSelcted:_model];
        }
...
//            height += (retSize.height > 20 ? retSize.height : 20) + 10;
        ...
        case EMMessageBodyTypeVideo:
        {
            ...
            if ([[model.message.ext objectForKey:@"title"] length] > 0) {
