    ...
    switch (self.model.bodyType) {
        case EMMessageBodyTypeText:
        {
            ...
            for (int i = 0 ; i < webPush.count ; i ++) {
                if ([self.model.text rangeOfString:webPush[i]].location != NSNotFound){
                    webPushBool = YES;
                    break;
                }
            }
            if (webPushBool == YES) {
                _bubbleView.textLabel.textColor = [UIColor blueColor];
            }else{
...
//    [self.avatarView sd_setImageWithURL:[NSURL URLWithString:model.avatarURLPath] placeholderImage:[UIImage imageNamed:@"chatListCellHead"]];
