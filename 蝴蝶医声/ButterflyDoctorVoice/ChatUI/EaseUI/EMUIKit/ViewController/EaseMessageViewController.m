...
//#define Gato_Height_548_(s) Gato_Height / 548 * s
...
@property (nonatomic) BOOL isMessageClass;//当前页面
@property (nonatomic, strong) NSMutableArray *atTargets;
@property (nonatomic, strong) NSString * ChatMessageId;//记录上一条存储的messageId

@property (nonatomic, strong) id<IMessageModel> nowVoiceMessageModel;//记录当前播放的语音
@end
...
- (void)viewDidLoad {
    ...
    if ([self _messageTypeFromConversationType] == EMChatTypeChat) {
        topViewHieght = Gato_Height_548_(70);
    }else{
        topViewHieght = 0;
    }
    ...
    if ([[GatoMethods getiPhoneType] isEqualToString:@"iPhone X"]) {
        iPhoneXHeight = Gato_Height_548_(30);
    }
    
//    EMChatToolbarType barType = EMChatToolbarTypeGroup;
    
    self.chatToolbar = [[EaseChatToolbar alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - chatbarHeight - iPhoneXHeight, self.view.frame.size.width, chatbarHeight) type:barType];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(MessageBackgroundCleanDisk)
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    self.isMessageClass = NO;
    self.isViewDidAppear = NO;
//    [[EaseMessageReadManager defaultManager] stopMessageAudioModel];
    [[EMCDDeviceManager sharedInstance] disableProximitySensor];
    [self _audioMessageCellSelected:self.nowVoiceMessageModel];
}
...
- (void)setChatToolbar:(EaseChatToolbar *)chatToolbar
{
    ...
    tableFrame.origin.y = topViewHieght;
    tableFrame.size.height = self.view.frame.size.height - _chatToolbar.frame.size.height - topViewHieght - iPhoneXHeight;
    ...
    if ([chatToolbar isKindOfClass:[EaseChatToolbar class]]) {
        ...
        UIImage * imagewc = [UIImage imageNamed:@"医生文章"];
        UIImage * imageqq = [UIImage imageNamed:@"医生文章"];
        [self.chatBarMoreView insertItemWithImage:imagewc highlightedImage:imageqq title:@"医生文章"];
        ...
        [self.chatBarMoreView updateItemWithImage:[UIImage imageNamed:@"xiangjiImage"] highlightedImage:[UIImage imageNamed:@"xiangjiImage"] title:@"相机" atIndex:2];
        [self.chatBarMoreView updateItemWithImage:[UIImage imageNamed:@"图片"] highlightedImage:[UIImage imageNamed:@"图片"] title:@"图片" atIndex:0];
    }
}
...
- (void)_audioMessageCellSelected:(id<IMessageModel>)model
{
    ...
    self.nowVoiceMessageModel = model;
    _scrollToBottomWhenAppear = NO;
...
#pragma mark - 切换到后台 停止语音
-(void)MessageBackgroundCleanDisk
{
    NSLog(@"开始运行切换到后台关闭语音");
    [self _audioMessageCellSelected:self.nowVoiceMessageModel];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (strongSelf) {
                    ...
                    if (self.conversation.type == EMConversationTypeChat){
                        if (self.ChatMessageId.length < 1) {
                            for (int i = 0 ; i < self.dataArray.count ; i ++) {
                                id model = self.dataArray[i];
                                if ([model isKindOfClass:[EaseMessageModel class]]) {
                                    EaseMessageModel * messageModel = model;
                                    if (messageModel.isSender) {
                                        self.ChatMessageId = [messageModel.message.ext objectForKey:@"messageId"];
                                    }
                                }
                            }
                        }
                    }
                    NSLog(@"self.ChatMessageId %@",self.ChatMessageId);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        NSDictionary * modelEXT = model.message.ext;
        
//        NSLog(@"ModelExt %@",modelEXT);
        
        if ([[modelEXT objectForKey:@"Tip"] isEqualToString:@"1"]) {
            NSLog(@"我收到了Tip提示 该显示了");
            ImMessageOneTipTableViewCell * cell = [ImMessageOneTipTableViewCell cellWithTableView:tableView];
            [cell setValueWithTitle:[modelEXT objectForKey:@"extmsg"]];
            return cell;
        }
...
- (void)messageCellSelected:(id<IMessageModel>)model
{
    ...
    switch (model.bodyType) {
        ...
        {
            if (_delegate && [_delegate respondsToSelector:@selector(tapWithMessageTextForModel:)]) {
                [_delegate tapWithMessageTextForModel:model];
                return;
            }
        ...
        {
            if (_delegate && [_delegate respondsToSelector:@selector(tapWithMessageImageForModel:)]) {
                [_delegate tapWithMessageImageForModel:model];
            }
...
- (void)statusButtonSelcted:(id<IMessageModel>)model withMessageCell:(EaseMessageCell*)messageCell
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(redButtonForModel:)]) {
        [_delegate redButtonForModel:model];
    }
    
    __weak typeof(self) weakself = self;
    [[[EMClient sharedClient] chatManager] resendMessage:model.message progress:nil completion:^(EMMessage *message, EMError *error) {
        if (!error) {
            self.ChatMessageId = model.messageId;
...
- (void)LongAvatarViewSelcted:(id<IMessageModel>)model
{
    if (_delegate && [_delegate respondsToSelector:@selector(messageViewController:didSelectLongAvatarMessageModel:)]) {
        [_delegate messageViewController:self didSelectLongAvatarMessageModel:model];
...
- (void)chatToolbarDidChangeFrameToHeight:(CGFloat)toHeight
{
    [UIView animateWithDuration:0.3 animations:^{
        ...
        rect.origin.y = topViewHieght;
        rect.size.height = self.view.frame.size.height - toHeight - topViewHieght - iPhoneXHeight;
...
- (void)didFinishRecoingVoiceAction:(UIView *)recordView
{
    ...
    if ([self getAVAuth] == 0 ) {
        [self.recordView removeFromSuperview];
        [self showHint:@"当前麦克风功能未授权，请前往设置界面授权"];
    }else if ([self getAVAuth] == 1){
        [self.recordView removeFromSuperview];
    }else{
        if ([self.delegate respondsToSelector:@selector(messageViewController:didSelectRecordView:withEvenType:)]) {
...
#pragma make - 接收消息时 接收ext额外消息 头像 名字等
...
//                if (![model.nickname isEqualToString:GATO_PHOTO]) {
//                    model.nickname = self.ToUserName;
//                    model.avatarURLPath = self.ToImageUrl;
//                    model.avatarImage = self.ToImage;
//                }else{
//                    model.avatarURLPath = self.MyImageUrl;
//                }
//            }
//            model.nickname = [message.ext objectForKey:kChatUserNick];
//            model.avatarURLPath = [message.ext objectForKey:kChatUserPic];
//            [UserCacheManager save:message.ext];
//            model.message = [message.ext objectForKey:kChatUserId];
        }
...
- (void)_sendMessage:(EMMessage *)message
{
    ...
    if (self.conversation.type == EMConversationTypeGroupChat){
        ...
        [self groupAndRoomSendMessage:message];
    }
...
- (void)chatSendMessage:(EMMessage *)message
{
    self.ChatMessageId = message.messageId;//重新记录最新的消息messageId；
    NSMutableDictionary * extDic = [NSMutableDictionary dictionaryWithDictionary:message.ext];
    [extDic setObject:message.messageId forKey:@"messageId"];
    if (self.PlayNumber) {
        [extDic setObject:self.PlayNumber forKey:@"replayCount"];
    }
    message.ext = extDic;
...
- (void)groupAndRoomSendMessage:(EMMessage *)message
{
    [self addMessageToDataSource:message
                        progress:nil];
...
- (void)sendTextMessage:(NSString *)text withExt:(NSDictionary*)ext
{
    ...
    if ([self.delegate respondsToSelector:@selector(didSendTextOver:WithText:)]) {
        [self.delegate didSendTextOver:message WithText:text];
    }
...
//
    ...
    if ([self.delegate respondsToSelector:@selector(didSendTextOver:WithimageData:)]) {
        [self.delegate didSendTextOver:message WithimageData:imageData];
    }
...
- (void)sendImageMessage:(UIImage *)image
{
    ...
    if ([self.delegate respondsToSelector:@selector(didSendTextOver:Withimage:)]) {
        [self.delegate didSendTextOver:message Withimage:image];
    }
    
}
- (void)sendImageMessage:(NSData *)imageData withExt:(NSDictionary*)ext
{
    id progress = nil;
    if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:progressDelegateForMessageBodyType:)]) {
        progress = [_dataSource messageViewController:self progressDelegateForMessageBodyType:EMMessageBodyTypeImage];
    ...
    if ([self.delegate respondsToSelector:@selector(didSendTextOver:WithimageData:WithEXT:)]) {
        [self.delegate didSendTextOver:message WithimageData:imageData WithEXT:ext];
    }
}
                             ...
                             duration:(NSInteger)duration
{
    ...
    if ([self.delegate respondsToSelector:@selector(didSendVidoeMessage:WithLocalPath:duration:)]) {
        [self.delegate didSendVidoeMessage:message WithLocalPath:localPath duration:duration];
    }
